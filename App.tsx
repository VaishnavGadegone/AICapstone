...
// [ACTION REQUIRED] Paste the URL of the woman image here:
...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden font-inter relative text-slate-900">
      
      {/* ============================================== */}
      {/* LEFT PANEL (Avatar) - Hidden on Mobile         */}
      {/* ============================================== */}
      <div className="hidden md:flex md:w-[35%] lg:w-[30%] h-full relative flex-col justify-end items-center z-0">
          {ASHA_AVATAR_URL ? (
            <img 
              src={ASHA_AVATAR_URL} 
              alt="Asha Avatar" 
              className="max-h-[90vh] w-auto object-contain object-bottom drop-shadow-2xl animate-in slide-in-from-bottom-10 duration-1000 z-10"
            />
          ) : (
            <div className="mb-32 p-8 text-center bg-white/60 backdrop-blur-md rounded-2xl border-2 border-dashed border-slate-300 shadow-sm mx-8">
               <p className="text-slate-500 font-bold mb-2">👩🏾 ASHA AVATAR</p>
               <p className="text-xs text-slate-400 leading-relaxed">
                 To see the woman image here:<br/>
                 1. Open <code>App.tsx</code><br/>
                 2. Paste URL into <code>ASHA_AVATAR_URL</code>
               </p>
            </div>
          )}
      </div>
      ...
      {/* ============================================== */}
      <div className="flex-1 flex flex-col h-full relative z-10 bg-white/40 backdrop-blur-md md:border-l border-white/50 shadow-2xl md:shadow-none">
          
          <EmergencyBanner />

          {/* Header */}
          <header className="flex-none px-4 py-3 z-10 w-full flex items-center justify-between border-b border-white/20 bg-white/20 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white/50 backdrop-blur-sm rounded-xl shadow-sm border border-white/20">
                {CUSTOM_LOGO_URL ? (
                  <img src={CUSTOM_LOGO_URL} alt="JanSathi Logo" className="w-9 h-9 sm:w-11 sm:h-11 object-contain" />
                ) : (
          ...
          {/* Input Area - Absolute Positioning inside Right Panel */}
          <div className="absolute bottom-0 left-0 right-0 p-4 z-20 pointer-events-none flex justify-center bg-gradient-to-t from-white/80 via-white/50 to-transparent pt-10">
